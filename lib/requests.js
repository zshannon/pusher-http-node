...
async function send(config, options) {
  ...
  const url = `${config.getBaseURL()}${path}?${await createSignedQueryString(
    config.token,
    {
      method,
      path,
...
async function createSignedQueryString(token, request) {
  if (typeof global !== "undefined" && global.PUSHER_createSignedQueryString) return global.PUSHER_createSignedQueryString(token, request)
  ...
  if (request.body) {
    params.body_sha256 = await util.getSHA256(request.body)
  }
