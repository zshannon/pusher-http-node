...
global.PUSHER_createSignedQueryString = (token, request) => {
	...
	if (request.body) {
		parameters.body_md5 = crypto.createHash('md5').update(request.body).digest('hex')
	}
	...
	const digest = crypto.createHash('sha256').update(Buffer.from(signData)).digest()
	...
	queryString +=
		'&auth_signature=' + `${Buffer.from(sigObject.signature).toString('hex')}`
